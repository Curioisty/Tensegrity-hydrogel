...
echo   region 6wall block  -10 110 -10 110  0 100
echo   fix w1  all wall/region 6wall lj126 1 1 2.5

echo  region  r_lower   block  0  100  0  100    0  20 
echo  region  r_upper   block  0  100  0  100    80 100


echo  group bm  type 2
echo  group AAm type 1 
echo  group alower region r_lower
echo  group aupper region r_upper
echo  group lower intersect bm alower
echo  group upper intersect bm aupper
...
echo  fix duanjian AAm bond/break  100  1  3.3  prob 0.05 ${rand1}
...
echo  variable  longforce equal f_sf2[3]-f_sf1[3]
echo  fix myat2 all ave/time 10 10 100 v_longforce file forcetube.dat


echo   fix m1 lower  move linear 0 0   0.1 units lattice
echo   fix m2 upper  move linear 0 0  -0.1 units lattice
...
echo  run 5000
echo  write_data pull7.data

echo  run 3000
echo  write_data pull8.data
